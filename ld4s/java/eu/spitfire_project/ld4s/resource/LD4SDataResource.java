...
public abstract class LD4SDataResource extends ServerResource{
	public static enum SparqlType {SELECT, CONSTRUCT, UPDATE, DESCRIBE, ASK};
...
//		resource2namedGraph.put("meas_prop", base+"meas_prop");
		...
		//use base graph for all resources 
		
		resource2namedGraph.put("link", base);
		resource2namedGraph.put("device", base);
		resource2namedGraph.put("tpp", base);
		resource2namedGraph.put("tps", base);
		resource2namedGraph.put("platform", base);
		resource2namedGraph.put("meas_capab", base);
		resource2namedGraph.put("meas_prop", base);
	...
	protected void doInit() throws ResourceException {
		...
		//		}
		this.ld4sServer = (Server) Server.getServerInstance();
	...
	protected Model initModel(Model model, String rdfFileName) {
		if (model == null) {
			...
			// read the RDF file
			model.read("http://spitfire-project.eu/ontology.owl");
			//model.re
		}
		return model;
	}
	...
	private Model handleGenericResources(Model model,
		...
		while (rit.hasNext()){
			...
			if (res.getURI() == null){//in case of blank nodes
				...
				if (res.getURI().startsWith(Server.getHostName())){
...
//		testSparqlPrint(namedModel);
	...
	public Resource addLinkedData(Resource resource,
		...
		switch(searchType){
		case ALL:
			searchobj = new GenericApi(Server.getHostName(), 
					context, this.user, resource);
			break;
		case PEOPLE:
			searchobj = new PeopleApi(Server.getHostName(), 
					context, this.user, resource);
			break;
		case WEATHER:
			searchobj = new WeatherApi(Server.getHostName(), 
					context, this.user, resource);
			break;
		case LOCATION:
			searchobj = new LocationApi(Server.getHostName(), 
					context, this.user, resource);
			break;
		case FEATURE: //searched in DBpedia ONLY
			searchobj = new EncyclopedicApi(Server.getHostName(), 
					context, this.user, resource);
			break;
		case UNIT:
			searchobj = new UomApi(Server.getHostName(), 
	...
	protected Resource addWeatherForecast(Resource resource) throws Exception{
		...
		String item_uri = getResourceUri(Server.getHostName(), "resource/weather_forecast", id);
		Resource item_resource = resource.getModel().createResource(item_uri);
		Context con = new Context(Server.getHostName());
	...
	protected Resource addLocation(Resource resource, String location_name, String[] location_coords){
		...
		if (location_name != null){
			item_uri = getResourceUri(Server.getHostName(), "resource/location", location_name);
			...
			item_uri = getResourceUri(Server.getHostName(), "resource/location", lc);
		}else{
			return resource;
		}
		Resource item_resource = resource.getModel().createResource(item_uri);
	...
	protected Resource addFoi(Resource resource, String foi) throws Exception{
		if (foi.contains("weather")){
			...
			String item_uri = getResourceUri(Server.getHostName(), "resource/property", foi);
			Resource item_resource = resource.getModel().createResource(item_uri);
			Context con = new Context(Server.getHostName());
			con.setThing(foi);
	...
	protected Resource addObservedProperty (Resource resource, String observed_property,
			Property prop)
	throws Exception{
		String item_uri = getResourceUri(Server.getHostName(), "resource/property", observed_property);
		Resource item_resource = resource.getModel().createResource(item_uri);
		Context con = new Context(Server.getHostName());
...
//		});
	...
	protected Resource addUom(Resource resource, String uom) throws Exception{
		String item_uri = getResourceUri(Server.getHostName(), "resource/uom", uom);
		Resource item_resource = resource.getModel().createResource(item_uri);
		Context con = new Context(Server.getHostName());
		con.setThing(uom);
	...
	protected Resource addPerson(Resource resource, Person item, Property prop) throws Exception{
		...
		String item_uri = getResourceUri(Server.getHostName(), "resource/people", id);
		Resource item_resource = resource.getModel().createResource(item_uri);
		Context con = new Context(Server.getHostName());
		con.setPerson(item);
	...
	protected Object sparqlExec(String queryString, 
			SparqlType type){
		Object ret = null;
...
//		testSparqlPrint(namedModel);
	...
	protected Resource createNewType(String type){
		...
		Resource newtype = vocab.createResource(Server.getHostName()
