...
public class NetworkDeviceFilter extends ReasonerManager{
	...
	public static Model applyFilter(ActuatorDecision obj, 
		...
		String selectBase = "select ?sensor where {graph ";
		if (namedGraphUri != null){
			selectBase += " <"+namedGraphUri+"> {";
		}else{
			selectBase += " ?graph {";
		}
		String whereBase = " <" +actuatorProp+"> <"+SptCtVocab.PROPERTY_OF.getURI()+"> ?appDomain ." +
				"?sensor <"+SptVocab.OBSERVED_PROPERTY.getURI()+"> ?obsProp ." +
				"?obsProp <"+SptCtVocab.PROPERTY_OF.getURI()+"> ?appDomain ." ;
		...
		String whereBase1 = "?actuator <"+SptSnVocab.ACTUATED_PROPERTY.getURI()+"> <" +actuatorProp+"> ." +
				"<" +actuatorProp+"> <"+SptCtVocab.TARGET.getURI()+"> ?target ." +
				"?sensor <"+SptVocab.OBSERVED_PROPERTY.getURI()+"> ?obsProp ." +
				"?obsProp <"+SptCtVocab.TARGET.getURI()+"> ?target ." ;


		String queryString = selectBase + whereBase + " union {"+whereBase1+"}}}";
